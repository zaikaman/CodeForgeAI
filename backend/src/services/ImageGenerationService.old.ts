  ...
  for (let cycle = 0; cycle < maxCycles; cycle++) {
    for (let i = 0; i < numKeys; i++) {
      ...
      try {
        ...
        // Generate image - wrap in another try-catch to log raw error
        let imageBuffer: Buffer;
        try {
          imageBuffer = await generateImageWithKey(prompt, currentKey, finalOptions);
        } catch (innerError: any) {
          // Log raw error object with all properties
          console.error(`🔴 Raw error from generateImageWithKey:`);
          console.error(JSON.stringify(innerError, Object.getOwnPropertyNames(innerError), 2));
          console.error(`🔴 Error type: ${typeof innerError}, constructor: ${innerError?.constructor?.name}`);
