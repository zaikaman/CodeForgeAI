      ...
      try {
        if (file.path.endsWith('.ts') || file.path.endsWith('.tsx')) {
          ...
          for (const diagnostic of diagnostics) {
            ...
            const fixable = isSimpleFix
            const fixStrategy = isSimpleFix ? 'ai_fixer' : 'manual'
  ...
  private detectCommonMistakes(file: GeneratedFile, sourceFile: ts.SourceFile): ValidationError[] {
    ...
    if (file.content.includes('Router.useRouter()')) {
      errors.push({
        ...
        file: file.path,
        message:
          "Property 'useRouter' does not exist on type '() => NextRouter'. Import and use useRouter() directly.",
        fixable: true,
        fixStrategy: 'syntax_fixer',
      })
    }
    ...
    while ((match = nestedTemplateRegex.exec(file.content)) !== null) {
      ...
      errors.push({
        ...
        line: line + 1,
        message:
          'Potentially problematic nested template string. May cause "Unterminated string literal" error.',
        fixable: true,
        fixStrategy: 'syntax_fixer',
      })
    }
    ...
    if (
      file.content.includes('useRouter()') &&
      !file.content.match(/import.*useRouter.*from ['"]next\/router['"]/)
    ) {
      errors.push({
        severity: 'critical',
        layer: 'static',
    ...
    if (
      ...
      !file.content.match(/import.*useState.*from ['"]react['"]/)
    ) {
      errors.push({
        severity: 'critical',
        layer: 'static',
      ...
      if (doubleQuoteStart % 2 !== 0 || singleQuoteStart % 2 !== 0) {
        ...
        if (hasUnterminatedDouble || hasUnterminatedSingle) {
          errors.push({
            ...
            line: i + 1,
            message:
              'Unterminated string literal detected. Check for missing closing quote or use \\n for newlines in JSON strings.',
            fixable: true,
            fixStrategy: 'syntax_fixer',
          })
        }
    ...
    for (const file of files) {
      ...
      if (file.content.includes('console.log') && !file.path.includes('test')) {
        errors.push({
          ...
          file: file.path,
          message:
            'console.log() found in production code. Consider removing or using proper logging.',
          fixable: true,
      ...
      for (const imp of imports) {
        ...
        if (!exists) {
          errors.push({
            ...
            fixable: true, // Changed from false - AI can create missing files
            fixStrategy: 'ai_fixer', // Needs AI to create missing files
          })
        }
      }
    }
        ...
        if (pkg.type === 'module') {
          ...
          if (nextConfig && nextConfig.content.includes('module.exports')) {
            errors.push({
              ...
              file: 'next.config.js',
              message:
                'package.json uses "type": "module" but next.config.js uses CommonJS. Use "export default" or rename to .cjs',
              fixable: true,
              fixStrategy: 'module_converter',
            })
          }
        ...
        if (hasReact && hasVite) {
          ...
          if (!hasPluginReact) {
            errors.push({
              ...
              file: 'package.json',
              message:
                'React + Vite project missing "@vitejs/plugin-react" in devDependencies. Add "@vitejs/plugin-react": "^4.0.0"',
              fixable: true,
              fixStrategy: 'dependency_fixer',
            })
          }
